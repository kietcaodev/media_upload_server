...
public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    private static void SeedData(ModelBuilder model)
    {
        ...
        model.Entity<SystemSetting>().HasData(
            new SystemSetting { Id = 1,  Key = "nas.upload_dir",               Value = "/opt/media-upload/uploads", Description = "Thư mục lưu file upload trên NAS",           HotReload = true, UpdatedAtUtc = seedTimestamp },
            new SystemSetting { Id = 2,  Key = "nas.logs_dir",                 Value = "/opt/media-upload/logs", Description = "Thư mục lưu log",                            HotReload = true, UpdatedAtUtc = seedTimestamp },
            ...
            new SystemSetting { Id = 16, Key = "nas.local_staging_dir",        Value = "/opt/media-upload/staging", Description = "Thư mục lưu tạm local trước khi chuyển sang NAS (ngoài time window)", HotReload = true, UpdatedAtUtc = seedTimestamp },
            ...
            new SystemSetting { Id = 7,  Key = "upload.allowed_extensions",    Value = ".mp4,.avi,.mov,.mkv,.flv,.wmv,.webm,.3gp,.mp3,.wav,.ogg,.aac,.flac,.m4a,.wma,.jpg,.jpeg,.png,.gif,.bmp,.webp,.tiff,.tif,.ico,.heic,.heif", Description = "Các đuôi file video/audio/ảnh được phép", HotReload = true, UpdatedAtUtc = seedTimestamp },
            ...
            new SystemSetting { Id = 14, Key = "cors.allowed_origins",         Value = "https://103.104.123.126:8443,http://localhost:5173", Description = "Danh sách origin CORS (phân cách bằng dấu phẩy)", HotReload = true, UpdatedAtUtc = seedTimestamp },
        ...
        const string adminRawToken = "MediaUploadAdmin2026!ChangeMe";
        ...
        const string adminHashedSecret = "$2a$12$9ablaBC2kGW/GEpm/Lml3.v.956o.gNZY68.jUg.I9Ps/tvoEIRcO";
